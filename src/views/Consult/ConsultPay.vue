...
import { createConsultOrder, getConsultOrderPayUrlAPI, getConsultOrderPre } from '@/services/consult';
...
onMounted(() => {
  ...
  if(!valid) {
    return showDialog({
      title: '温馨提示',
      message: '问诊信息不完整，请重新填写，如有未支付的问诊订单可以在问诊记录中继续支付！',
...
const closeActionShit = () => {
  return showConfirmDialog({
    title: '温馨提示',
    message: '取消支付将无法获得医生回复，医生接诊名额有限，你确认取消吗？',
...
<template>
  ...
  <div class="consult-pay-page" v-else>
    <cp-nav-bar title="支付" />
    <!-- 骨架屏 -->
    <van-skeleton title avatar :row="3" style="margin-top: 18px;" />
    <van-skeleton title avatar :row="3" style="margin-top: 18px;" />
    <van-skeleton title avatar :row="3" style="margin-top: 18px;" />
  </div>
</template>
